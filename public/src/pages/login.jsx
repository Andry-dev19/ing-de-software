        ...
        <section className="h-screen flex relative">
            ...
            <div className='relative z-10 flex-grow flex justify-center items-center' >
                <Form className="bg-black/30 text-white" event={isCreateAcount ? sendNewUser : sendForm}>
                    ...
                    <p className='mt-4 text-right'>{!isCreateAcount ? "No Tienes Cuenta? " : "No Tienes Cuenta? "} <span className='font-semibold cursor-pointer' onClick={() => setIsCreateAcount(!isCreateAcount)}>{isCreateAcount ? "Iniciar Sesion" : "Crear Cuenta"}</span></p>
                </Form>
            </div>
        </section>
    )
